<p align="center">
  ...
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/version-2.8.0-yellow"></a>
</p>
...
- [Версионная библиотека шагов Vanessa](#версионная-библиотека-шагов-vanessa)
- [Библиотеки определений проекта](#библиотеки-определений-проекта)
...
## Версионная библиотека шагов Vanessa

Для IntelliSense, hover и диагностики расширение выбирает встроенные шаги по точной версии Vanessa Automation в каждой папке workspace:

1. Если задано `kotTestToolkit.steps.vanessaVersion`, используется указанная версия формата `X.Y.Z.W`.
2. Иначе KOT берет путь `kotTestToolkit.runVanessa.vanessaEpfPath`, находит рядом `docs/Changelog.md` и читает версию из его первого заголовка версии.
3. Для найденной версии загружается неизменяемый каталог, сгенерированный из официального `locales/Steps/Templates/en/Ext/Template.xml`. Проверенный каталог сохраняется в локальном кэше расширения и повторно не скачивается.

Первое открытие файла не ждет сеть: если точного каталога еще нет в кэше или сеть недоступна, сразу используется встроенный `res/steps.htm`, а точная версия подхватывается в фоне. Команда `KOT - Refresh steps library` принудительно проверяет текущую версию и обновляет общий источник для IntelliSense, hover и диагностики. `kotTestToolkit.steps.externalUrl` оставлен только как явно включаемый fallback для прежнего HTML-формата.

Пользовательские шаги проекта и экспортные сценарии не входят во встроенный каталог: они индексируются из библиотек активного профиля, как описано ниже.

## Библиотеки определений проекта

KOT объединяет вложенные YAML-сценарии, встроенные шаги Vanessa и локальные определения проекта в одном IntelliSense, hover, F12 и Shift+F12. Для локальных библиотек используются параметры **активного профиля** текущей папки workspace. Корни выбираются в таком порядке:

1. эффективный дополнительный параметр Vanessa `librarycatalogs` / `КаталогиБиблиотек`;
2. параметр сборки `Libraries`;
3. параметр сборки `VanessaLibraries`.

Относительные пути и токены `#SourcesPath` / `#Libraries` разрешаются относительно своей папки workspace. При переключении активного профиля старое сканирование отменяется, наблюдатели пересоздаются, а редактор получает определения только нового профиля.

Поддерживаются два исходных формата:

- `.feature` на русском или английском языке: экспортируется вся функциональность с тегом `@ExportScenarios` либо отдельные сценарии с этим тегом;
- `.bsl`: статические регистрации `ДобавитьШагВМассивТестов` в исходном модуле пользовательской библиотеки. Динамически вычисляемые шаблоны безопасно пропускаются с предупреждением.

Команды `KOT - Create exported scenario` и `KOT - Create user step` доступны из палитры, quick fix неизвестного вызова и меню создания Test Manager. Экспортный сценарий можно добавить в существующий `.feature` или создать новый; добавление отсутствующего `@ExportScenarios` требует подтверждения. Пользовательский шаг можно добавить в существующий безопасно разобранный BSL-модуль или создать новую исходную библиотеку:

```text
<LibraryRoot>/
├── step_definitions-src/<LibraryName>/
│   ├── Обработка.xml
│   └── Обработка/Forms/Форма/Ext/Form/Module.bsl
└── step_definitions/<LibraryName>.epf
```

Для новой библиотеки KOT копирует метаданные `lib/TemplateEpfUF` из Vanessa, найденной по `VanessaFolder`, `VanessaDir`, `VanessaPath` либо `kotTestToolkit.runVanessa.vanessaEpfPath`. Создание исходников не требует установленной платформы 1С. Команда `KOT - Build user-step library` запускается отдельно, требует настроенную платформу и служебную файловую базу, собирает временный EPF через Designer и заменяет целевой файл только после успешной проверки результата. При ошибке существующий EPF сохраняется.

Существующие библиотеки, представленные только бинарным `.epf` без `step_definitions-src`, пока недоступны для IntelliSense и навигации. Автоматическая распаковка останется отдельной opt-in возможностью будущей версии.

Индекс запускается в фоне, хранит проверенный кэш по размеру/mtime и обновляет только изменившийся `.feature` или `.bsl`. Если внешний инструмент меняет файлы без события файловой системы, сохраните файл повторно или выполните `Developer: Reload Window`; удалять кэш вручную обычно не требуется.
