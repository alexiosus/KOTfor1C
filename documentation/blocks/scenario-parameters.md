...
При вводе `[` в `ТекстСценария` IntelliSense показывает параметры, уже объявленные в `ПараметрыСценария` текущего YAML-сценария.
...
Hover по `[ParameterName]` в `ТекстСценария` показывает это значение и дает быстрое действие для его редактирования через стандартный `InputBox`.
