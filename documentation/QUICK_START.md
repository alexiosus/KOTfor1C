...
3. При необходимости включите поиск по главным сценариям кнопкой с лупой в шапке Test Manager.
...
Другие AI-сценарии работы:

- `KOT - Generate tester-friendly configuration diff report with AI` — отчет для тестировщика по diff конфигурации относительно главной ветки;
- `KOT - Review changed tests with AI` — ревью измененных автотестов по diff тестов, diff конфигурации и опциональному `UserStory`.

## 6) Первый build

1. В Test Manager отметьте нужные главные сценарии.
...
2. В верхней части Test Manager есть кнопка `Open Vanessa` с выбором сценария и режима:
...
4. KOT поднимет bridge-сессию:
   - рабочая база откроется в `TestClient`;
   - служебная startup-ИБ откроет EPF-мост в `TestManager`;
   - мост начнет писать snapshot активной формы в `kotTestToolkit.formExplorer.snapshotPath`.
5. После появления snapshot-а можно:
   - переключать `manual` / `auto`;
   - делать `Refresh`;
   - использовать `Locator`;
   - переходить к исходному `Form.xml`.
6. Пока bridge-сессия не закрыта, `Start infobase` останется недоступной. Для запуска другой базы сначала закройте текущие окна `TestClient`/моста.
