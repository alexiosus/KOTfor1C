...
- YAML-сценарии остаются в `yaml` language mode, поэтому `editor.formatOnSave` не выравнивает весь текст сценария как обычный блок `feature`. ([#39](https://github.com/kakoytochelik/KOTfor1C/issues/39))
...
- Исправлена подстановка строки подключения к серверной базе при запуске теста ([#40](https://github.com/kakoytochelik/KOTfor1C/issues/40)).
...
- Добавлен небольшой набор демо тестов ([#28](https://github.com/kakoytochelik/KOTfor1C/issues/28)).

# 2.6.0

### AI-описание тестов

- Добавлена функция генерации содержимого блока `KOTМетаданные.Описание` через подключаемую LLM. ([#30](https://github.com/kakoytochelik/KOTfor1C/issues/30))
...
- Добавлен каталог платформ 1С и отдельный менеджер платформ. Его можно открыть через настройки или из меню действий Test Manager. ([#34](https://github.com/kakoytochelik/KOTfor1C/issues/34))
...
- Добавлен менеджер эталонных баз для редактирования `bases.yaml` как источника `ModelDBSettings` в параметрах СППР. ([#35](https://github.com/kakoytochelik/KOTfor1C/issues/35))
...
- Для установки и запуска Form Explorer добавлены режимы `direct`, `cfe` и `target`: ([#37](https://github.com/kakoytochelik/KOTfor1C/issues/37))
...
- В менеджере параметров появились профили наборов настроек с автосохранением. ([#36](https://github.com/kakoytochelik/KOTfor1C/issues/36))
...
- В контекстное меню YAML-сценария добавлена команда открытия папки `files`. ([#33](https://github.com/kakoytochelik/KOTfor1C/issues/33))
