...
The parser is deliberately strict about the surrounding 1C spreadsheet XML shape: malformed rows, unexpected cell counts, a missing header, duplicate IDs, missing source metadata, structurally invalid placeholders, an unexpectedly small catalog, and a step-count drop greater than 10% from the newest preceding published version fail generation. Placeholder validation requires a positive non-zero number, a whitespace separator, and matching single or double quotes. It intentionally does not require identical numbering between the RU and EN variants because the official `1.2.043.28` table already contains several translation-numbering defects. Upstream also contains legitimate repeated translated category labels (14 repeated English patterns in `1.2.043.28`), so per-language duplicates are retained and recorded in the generation report instead of invalidating the catalog. No BSL parser or heuristic reconstruction of `ДобавитьШагВМассивТестов` calls is introduced. The action writes a machine-readable generation report alongside the catalog.
